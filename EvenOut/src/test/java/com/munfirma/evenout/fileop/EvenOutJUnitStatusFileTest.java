...
public class EvenOutJUnitStatusFileTest {
    ...
    @Test
    public void writeNReadPersonsTest() throws IOException {
        List<Person> persons = group.getPersons();
        group2 = new CostGroup("Cost Group 1");

        assertEquals(group2.getPersons(), persons);
    }

    @Test
    public void writeNReadPaymentsTest() throws IOException {
        ...
        for (Person p : group2.getPersons()) {
            paid2.add(group2.getPaid(p));
            cost2.add(group2.getCost(p));
        }        
